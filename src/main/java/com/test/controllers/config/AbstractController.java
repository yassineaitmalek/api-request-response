...
public interface AbstractController {
  ...
  public default ApiDataRequest request() {
    return Optional.ofNullable(RequestContextHolder.getRequestAttributes())
        ...
        .map(ApiDataRequest::of)
        .orElseThrow(() -> new ClientSideException("api request is not proper"));
  }

  public default <T> ResponseEntity<ApiDataResponse<T>> ok(Supplier<T> supplier, ApiDataRequest apiDataRequest) {
    ApiDataResponse<T> response = ApiDataResponse.<T>builder()
        .data(supplier.get())
        .start(apiDataRequest.getStart())
        .httpStatus(HttpStatus.OK.value())
        .status(HttpStatus.OK.toString())
        .build();

    response.setEnd(LocalDateTime.now());
    response.setDuration(Duration.between(response.getStart(), response.getEnd()).getSeconds());
